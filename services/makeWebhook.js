async function makeWebhook(status, student) {
  const url = status === 'paid'
    ? process.env.MAKE_WEBHOOK_PAID
    : process.env.MAKE_WEBHOOK_FAILED;

  if (!url || url.includes('xxxxxxxxxx')) {
    console.warn('[Make.com] Webhook URL not configured for status:', status);
    return;
  }
